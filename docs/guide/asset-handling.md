...
`./handbook.pdf` 只是告诉浏览器“点击后去这里找文件”，不会告诉构建工具“把这个文件带进最终网站”。

本地开发时，服务器还能直接读到电脑里的这个文件，所以链接可能看起来能用。生产构建时，只有图片、CSS、`import` 等真正引用到的资源，或公共资源目录里的文件，才会保证被复制出去。文件没有被复制时，读者点击链接后就会看到找不到文件的页面。
:::
